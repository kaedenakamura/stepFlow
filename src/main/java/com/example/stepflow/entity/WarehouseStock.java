...
@Table(name = "warehouse_stock")//「MySQLの中の "warehouse_stock" というテーブルを使います」という指定
@Data//「Getter/Setterを自動で作ってください」という命令.
public class WarehouseStock {
    @Id//「主キーを決める」という印
    @GeneratedValue(strategy = GenerationType.IDENTITY)//「番号は自動でプラス1」という指定
    @Column(name = "warehouse_stock_id")//「MySQL側では warehouse_stock_id という列名ですよ」という指定
    private Integer warehouseStockId;//倉庫在庫ID
    ...
    private Integer warehouseStock = 0;//在庫数 0=在庫なし 1=在庫あり
